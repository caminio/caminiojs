...
Server.prototype.webSetup = function webSetup( callback ){
  ...
  this.app.express.use(express.cookieParser('caminio sec'));
  ...
  // use jsonp callbacks
  this.app.express.set("jsonp callback", true);

  this.app.express.use(flash())
  this.app.express.locals.resolvePath = router.resolve;
  this.app.express.locals.caminio = {};
  this.app.express.locals.caminio.version = JSON.parse( fs.readFileSync(__dirname+'/../../package.json') ).version;
  this.app.express.locals.caminio.env = this.app.environment;
  this.app.express.locals.caminio.config = this.app.config;
  ...
  // security
  // TODO Find a way to ommitt per action
  // or provide a middleware to enable at least
