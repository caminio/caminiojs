...
Controller.prototype.addLocals = function addLocals(){
  ...
  this.before( 
      function( req, res, next ){
        ...
        res.locals.csrf = ''; // TODO: find a better solution for allowing posts ( self.app.environment !== 'test' ? req.session.req.csrfToken() : '' );
        res.locals.remoteHost = 'http'+(req.connection.encrypted ? 's' : '')+'://'+req.headers.host;
        res.locals.currentUser = req.user;
        res.locals.controllerName = self.name;
        res.locals.controllerNameWithoutNS = self.name.split('/')[self.name.split('/').length-1];
        ...
        if( req.user.lang )
          req.i18n.lng( req.user.lang );
        
        // handle domain loading
        req.session.current_domain_id = req.session.current_domain_id || (req.user.domains.length ? req.user.domains[0] : null);
        if( req.param('change_domain_id') ){ req.session.current_domain_id = req.param('change_domain_id'); }

        if( req.session.current_domain_id ){
          mongoose.models.Domain.findOne({ _id: req.session.current_domain_id }, function( err, domain ){
            if( err || !domain ){ 
              logger.error('controller', 'could not found domain from req.session_current_domain_id');
              return next(err); 
            }
          ...
          next('no_domain_selected_in_controller');
        }
 ...
 *
 * @class Controller
 * @method infiltratreResWithcaminio
**/
Controller.prototype.infiltrateResWithcaminio = function infiltrateResWithcaminio(){
  var self = this;
  this.before( 
      function( req, res, next ){
...
Controller.prototype.before = function before(){
  ...
  if( !foundFn )
    throw Error( '[caminio] ' + this.name + ': call of before hook without passing a function. This might not be what you are after (?)' )
