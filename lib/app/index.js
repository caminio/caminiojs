...
Caminio.prototype.init = function initCaminio( options ){
  ...
  async.auto({
    ...
    'initServer': ['collectHooks', function( cb ){ self.server = new Server(); cb(); } ],
    'collectConfig': [ 'initServer', loader.collectConfig ],
...
Caminio.prototype.run = function runCaminio( modules ){
  ...
  new Gear({ api: true, absolutePath: dirname(caller.filename), name: basename(dirname( caller.filename )) });
