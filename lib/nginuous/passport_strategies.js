  ...
  module.exports = function(){
    ...
    passport.use(new LocalStrategy(
      function(username, password, done) {
        nginuous.orm.models.User.findOne({ email: username }, function( err, user ){
          if( err || !user )
    ...
    passport.deserializeUser(function(id, done) {
      nginuous.orm.models.User.findOne({ id: id }, done );
    });
    ...
    passport.use('consumer', new ConsumerStrategy(
          ...
          function(consumerKey, done) {
            nginuous.orm.models.AuthToken.create({ token: consumerKey }, function( err, client ){
      ...
      function(requestToken, done) {
        nginuous.orm.models.RequestToken.findOne({ token: requestToken}, function(err, token) {
          if (err) { return done(err); }
    ...
    passport.use('token', new TokenStrategy(
          ...
          // match.
          function(consumerKey, done) {
            nginuous.orm.models.AuthToken.findOne({ token: consumerKey }, function(err, client) {
              if (err) { return done(err); }
              if (!client) { return done(null, false); }
      ...
      function(accessToken, done) {
        nginuous.orm.models.AuthToken.findOne({ token: accessToken }, function(err, token) {
          if (err) { return done(err); }
          nginuous.orm.models.User.findOne({ id: token.user_id }, function(err, user) {
            if (err) { return done(err); }
