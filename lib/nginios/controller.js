...
Controller.prototype.addLocals = function addLocals(){
  ...
  this.before( 
      function( req, res, next ){
        ...
        res.locals.currentDomain = req.session.domain || ( req.user && req.user.domains && req.user.domains[0] && req.user.domains[0].domain );
        next();
      });
}
 ...
 **/
Controller.prototype.resolvePath = function resolvePath( controller, action ){
  controller = controller || this.name;
  return router.resolve( controller, action );
}
