...
function Application( options ){
  ...
  new Gear({
    ...
    app_paths: {
      v1: [
        { name: 'users', path: 'users', icon: 'user', require_manager: true, position: 2 },
        { name: 'dashboard', path: 'dashboard', icon: 'home', position: 0, action: '/nginios' },
        { name: 'domains', path: 'domains', icon: 'tasks', require_superuser: true, position: 3 }
      ]
    }
  });
