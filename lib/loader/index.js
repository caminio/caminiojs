...
function loader( caminio ){
  ...
  function collectConfig( cb ){
    ...
    _.each( caminio.gears, function( gear ){
      ...
      // views
      var viewEnginesConfig = join(configDir,'view_engines.js');
      if( fs.existsSync( viewEnginesConfig ) )
        _.merge( caminio.config.viewEngines, require(viewEnginesConfig).viewEngines );

      // use /public in production, /assets in development
      var publicPath = join( gear.paths.absolute, 'assets' );
      if( caminio.env === 'production' )
        publicPath = join( gear.paths.absolute, 'public' );
      if( fs.existsSync( publicPath ) )
        caminio.views.publicPaths.push( publicPath );
    ...
    _.each( caminio.gears, function( gear ){
      ...
      fs
        ...
        .forEach( function( file ){
          caminio.hooks.register( file, require( join(hooksDir,file) )( caminio ) );
        });
  ...
  function collectMiddleware( cb ){
    ...
    _.each( caminio.gears, function( gear ){
      ...
      var globalMwPath = join( gear.paths.absolute, 'config', 'global.js' );
      if( fs.existsSync(globalMwPath) )
        caminio.globalExpressActions = caminio.globalExpressActions.concat( require(globalMwPath)(caminio) );
  ...
  function collectRecursivePath( path, ns, stack, load, camelCase ){
    ...
    fs
      ...
      .forEach( function( file ){
        ...
        if( extname( file ) === '.js' ){
          var name = inflection.camelize( join( ns, util.capitalize(basename(file).replace('.js','')) ).replace(/\//g,'::') );
          // caminio.logger.debug('init controller', name, path, file );
