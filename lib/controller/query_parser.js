...
module.exports = function( caminio ){
  ...
  QueryParser.prototype.collectConditions = function collectConditions( val, key ){
    ...
    if( key === 'limit' ) return;
    if( key === '_' ) return;
    // was not necessary in the end, as 'translations.title' is a string, not a nested
    //if( typeof(val) === 'object' && !(val instanceof Array) ){
    //  cond[key] = {};
    //  for( var k in val ){
    //    var recursiveCond = this.collectConditions( val[k], k );
    //    if( recursiveCond[k] )
    //      cond[key][k] = recursiveCond[k];
    //  }
    //  return cond;
    //}
    ...
    else if( typeof(val) === 'string' && val.match(/^ne\(|^lt\(|^in\(|^gt\(|^lte\(|^lteDate\(|^gte\(|^gteDate\(/) )
