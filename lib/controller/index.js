...
module.exports = function( caminio ){
  ...
  function setControllerAuthPolicies( actions, controller, controllerAction ){
    ...
    _.each( controller._policies, function( actionFn, controllers ){
      ...
      return;
      if( controllers.replace(/[\ ]*/g,'').split(',').indexOf(controllerAction) < 0 )
        return
      caminio.logger.debug('adding policy for action', controllerAction, controllers.replace(/[\*\!\(\)\ ]*/g,'').split(',').indexOf(controllerAction),controllers.replace(/[\*\!\(\)\ ]*/g,'') );
   ...
   if( !('_beforeResponse' in controller) ) return actions;
    _.each( controller._beforeResponse, function( actionFn, controllers ){
      if( ( controllers[0] === '*' && controllers.replace(/[\*\!\(\)\ ]*/g,'').split(',').indexOf(controllerAction) < 0 ) || 
        controllers.replace(/[\ ]*/g,'').split(',').indexOf(controllerAction) >= 0 ){
        if( actionFn instanceof Array )
          actionFn.forEach(function(action){ actions.splice( actions.length-1, 0, action ); });
        else
          actions.splice( actions.length-1, 0, actionFn );
      }
    });
