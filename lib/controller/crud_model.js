...
module.exports = function( caminio ){
  ...
  CrudModel.prototype.update = function createUpdate(){
    ...
    if( !('update' in this.controller) )
      this.controller['update'] = [ updateDoc, getDoc, function( req, res ){
  ...
  CrudModel.prototype.destroy = function createDestroy(){
    ...
    if( !('destroy' in this.controller) )
      this.controller['destroy'] = function( req, res ){
        ...
        // need to get model, otherwise, hooks will be bypassed
        self.model.findOne( { _id: req.param('id') }, function( err, model ){
          if( err ){ return res.json( 500, { error: 'server_error', details: err }); }
          if( !model )
            return res.json(404, { error: 'not found', id: req.param('id') });
          if( 'deletedBy' in model )
            model.deletedBy = res.locals.currentUser;
