  ...
  CrudModel.prototype.index = function createIndex(){    
    ...
    if( !('index' in this.controller) )
      this.controller['index'] = function( req, res ){
        ...
        if( 'deletedBy' in self.model.schema.paths )
          q.q.where({ deletedBy: null });
...
=======
>>>>>>> a80e6c88b5d2c3c404498f4b1b51e47458b11ca6
  ...
  /**
   * 
   * creates a RESTORE method for the controller
   * existing routes will remain untouched
   *
   * @method restore [POST]
   * @private
   */
  CrudModel.prototype.restore = function createRestore(){

    var self = this;

    if( !('restore' in this.controller) )
      this.controller['restore'] = function( req, res ){

        caminio.models[self.model.modelName].restore( req.param('id'), function( err, model ){
          if( err ){ return res.json( 500, { error: 'server_error', details: err }); }
          req[self.underscoredName] = model;
          var result = getModelWithNamespace( req.header('namespaced'), 
                                              self.model.modelName, 
                                              JSON.parse(JSON.stringify( req[self.underscoredName] )) );
          if( req.header('sideload') )
    ...
    caminio.controller.createRoute( 'post', this.route+'/:id/restore', this.controllerName, 'restore' );
