...
var authServer = oauthorize.createServer();
...
authServer.deserializeClient(function(id, done) {
  nginuous.app.gears.nginuous.orm.models.User.findOne({id: id}, done);
});
...
var requestToken = [
  ...
  authServer.requestToken(function(client, callbackURL, done) {
    var token = utils.uid(8)
      , secret = utils.uid(32)
   
    nginuous.app.orm.models.RequestToken.create({ token: token, 
                                              client_secret: secret, 
                                              client_id: client.id, 
                                              callbackURL: callbackURL }, function( err ){
...
var accessToken = [
  ...
  authServer.accessToken(
    ...
    function(client, requestToken, info, done) {
      ...
      nginuous.app.orm.models.AuthToken.create({ token: token, 
                                              client_secret: info.userID, 
...
var userAuthorization = [
  ...
  authServer.userAuthorization(function(requestToken, done) {

    nginuous.app.orm.models.RequestToken.findOne({ token: token}, function(err, token) {
      if (err) { return done(err); }
      nginuous.app.orm.models.User.findOne({ id: token.user_id}, function(err, client) {
        if (err) { return done(err); }
...
var userDecision = [
  login.ensureLoggedIn(),
  authServer.userDecision(function(requestToken, user, res, done) {
    var verifier = utils.uid(8);
    nginuous.app.orm.models.RequestToken.count({ token: requestToken, user_id: user.id }, function(err, count) {
      if (err) { return done(err); }
      if( count !== 1 ) { return done('not_found') }
