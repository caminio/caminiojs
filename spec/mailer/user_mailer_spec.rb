...
describe 'user_mailer' do 
  ...
  context 'welcome' do
    ...
    before(:each) do
      ...
      @user2.locale = 'en'
      @user2.save
      UserMailer.welcome( @user, 'the link' ).deliver
    end
    ...
    it 'should send an email' do
      expect( ActionMailer::Base.deliveries.count ).to eq(1)
      UserMailer.welcome( @user2, 'other link' ).deliver
      expect( ActionMailer::Base.deliveries.count ).to eq(2)
    end
    ...
    it 'has the title of the message as subject' do
      reset = I18n.locale
      I18n.locale = @user.locale
      expect( ActionMailer::Base.deliveries.first.subject ).to eq( I18n.t('caminio_welcome') )
      ActionMailer::Base.deliveries.clear
      UserMailer.welcome( @user2, 'other link' ).deliver
      expect( ActionMailer::Base.deliveries.first.subject ).not_to eq( I18n.t('caminio_welcome') )
      I18n.locale = reset
    end
