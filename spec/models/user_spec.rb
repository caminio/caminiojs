...
describe V1::User do
  ...
  describe "create" do
    ...
    it { expect(user).to be_a(V1::User) }
    it { expect(user.username).to eq('johndoe') }
    it { expect(V1::User.where( username: 'johndoe' ).first ).not_to be(nil) }
    it { expect(V1::User.where( username: 'johndoe' ).first ).to be_a(V1::User) }
  ...
  describe "update" do
    ...
    it { expect(V1::User.find_by( username: 'test2' )).not_to be(nil) }
    it { expect(V1::User.find_by( username: 'test-update' )).to be(nil) }
  ...
  describe "delete" do
    ...
    it { expect(V1::User.find_by( username: 'test-delete')).to be(nil) }
  ...
  describe "api_key" do
    ...
    describe "creates a new access token" do
      ...
      it { expect( api_key ).to be_a(V1::ApiKey) }
      ...
      describe "invalidates old api_key if new is aquired" do
        ...
        it "only finds one valid token" do
          ...
          expect( V1::ApiKey.find_by( id: api_key.id ) ).to be(nil)
        end
